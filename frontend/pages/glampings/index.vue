...
async function search() {
  ...
  const response = await api<{ data: Glamping[] }>('/api/glampings', { query })
...
<template>
  <div>
    <h1 class="font-display text-2xl font-bold text-gray-900">Cari Glamping</h1>

    <form class="card mt-4 grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4" @submit.prevent="search">
      <input v-model="filters.q" type="text" placeholder="Cari nama glamping" class="field-input">
    ...
    <p v-else-if="glampings.length === 0" class="mt-6 text-gray-600">Tidak ada glamping yang cocok dengan pencarian.</p>

    <div v-else class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <NuxtLink
        v-for="glamping in glampings"
        :key="glamping.id"
        :to="`/glampings/${glamping.slug}`"
        class="card block overflow-hidden transition hover:shadow-md"
      >
        <div class="relative">
          <img
            v-if="glamping.images[0]"
            :src="glamping.images[0].url"
          ...
          <span class="badge absolute right-2 top-2 bg-black/60 text-white">{{ formatRupiah(glamping.base_price) }}</span>
        </div>
        <div class="bg-brand-brown/90 p-3 text-white">
          <h2 class="font-display font-semibold">{{ glamping.name }}</h2>
          <p class="text-xs text-white/80">{{ glamping.city }}</p>
        </div>
      </NuxtLink>
    </div>
  </div>
