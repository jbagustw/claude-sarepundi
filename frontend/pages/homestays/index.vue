...
async function search() {
  ...
  const response = await api<{ data: Homestay[] }>('/api/homestays', { query })
...
<template>
  <div>
    <h1 class="font-display text-2xl font-bold text-gray-900">Cari Homestay</h1>

    <form class="card mt-4 grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4" @submit.prevent="search">
      <input v-model="filters.q" type="text" placeholder="Cari nama homestay" class="field-input">
    ...
    <p v-else-if="homestays.length === 0" class="mt-6 text-gray-600">Tidak ada homestay yang cocok dengan pencarian.</p>
    ...
    <div v-else class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <div class="relative">
          ...
          <span class="badge absolute right-2 top-2 bg-black/60 text-white">{{ formatRupiah(homestay.base_price) }}</span>
        </div>
        <div class="bg-brand-brown/90 p-3 text-white">
          <h2 class="font-display font-semibold">{{ homestay.name }}</h2>
          <p class="text-xs text-white/80">{{ homestay.city }}</p>
        </div>
      </NuxtLink>
    </div>
  </div>
