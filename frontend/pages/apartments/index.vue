...
async function search() {
  ...
  const response = await api<{ data: Apartment[] }>('/api/apartments', { query })
...
<template>
  <div>
    <h1 class="font-display text-2xl font-bold text-gray-900">Cari Apartment</h1>

    <form class="card mt-4 grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4" @submit.prevent="search">
      <input v-model="filters.q" type="text" placeholder="Cari nama apartment" class="field-input">
    ...
    <p v-else-if="apartments.length === 0" class="mt-6 text-gray-600">Tidak ada apartment yang cocok dengan pencarian.</p>
    ...
    <div v-else class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <div class="relative">
          ...
          <span class="badge absolute right-2 top-2 bg-black/60 text-white">{{ formatRupiah(apartment.base_price) }}</span>
        </div>
        <div class="bg-brand-brown/90 p-3 text-white">
          <h2 class="font-display font-semibold">{{ apartment.name }}</h2>
          <p class="text-xs text-white/80">{{ apartment.city }}</p>
        </div>
      </NuxtLink>
    </div>
  </div>
