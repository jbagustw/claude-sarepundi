...
/**
 * Homestay-equivalent for Apartment bookings — flat base_price x nights,
...
class ApartmentAvailabilityService
{
     ...
     */
    public function evaluate(Apartment $apartment, CarbonImmutable $checkIn, CarbonImmutable $checkOut, int $guestCount, ?string $couponCode = null): array
    {
        $nights = $checkIn->diffInDays($checkOut);

        if ($guestCount > $apartment->capacity_guest) {
            return $this->unavailable("Kapasitas apartment maksimal {$apartment->capacity_guest} tamu.");
        }

        $hasConflict = Booking::where('bookable_type', Apartment::class)
            ->where('bookable_id', $apartment->id)
