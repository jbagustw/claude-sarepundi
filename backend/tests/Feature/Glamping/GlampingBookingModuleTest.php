...
class GlampingBookingModuleTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function publishedGlamping(User $mitra, array $overrides = []): Glamping
    {
        return $mitra->mitraProfile->glampings()->create(array_merge([
            'name' => 'Glamping Bukit Malang',
            'slug' => 'glamping-bukit-malang-'.uniqid(),
            'city' => 'Malang',
            'capacity_guest' => 4,
            'bedroom_count' => 2,
            'bathroom_count' => 1,
    ...
    private function paidBooking(User $user, Glamping $glamping, string $status, \DateTimeInterface $checkIn): Booking
    {
        $booking = Booking::create([
            'booking_code' => 'BK'.uniqid(),
            'user_id' => $user->id,
            'bookable_type' => Glamping::class,
            'bookable_id' => $glamping->id,
    ...
    public function test_availability_check_returns_correct_price_for_open_dates(): void
    {
        ...
        $glamping = $this->publishedGlamping($mitra, ['base_price' => 500000]);

        $response = $this->fromFrontend()->getJson(
            "/api/glampings/{$glamping->slug}/availability?check_in_date=2026-08-10&check_out_date=2026-08-13&guest_count=2"
        );
    ...
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra, ['capacity_guest' => 2]);

        $response = $this->fromFrontend()->getJson(
            "/api/glampings/{$glamping->slug}/availability?check_in_date=2026-08-10&check_out_date=2026-08-12&guest_count=5"
        );
    ...
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra);
        $user = $this->regularUser();
        $this->paidBooking($user, $glamping, 'dikonfirmasi', now()->addDays(10));

        $response = $this->fromFrontend()->getJson(
            "/api/glampings/{$glamping->slug}/availability?check_in_date=".now()->addDays(11)->format('Y-m-d')
    ...
    public function test_user_can_create_booking_for_a_glamping(): void
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra, ['base_price' => 500000]);
        $user = $this->regularUser();
        ...
        $response->assertJsonPath('data.total_price', 1500000);
        $response->assertJsonPath('data.bookable.type', 'glamping');
    ...
    public function test_cannot_book_a_glamping_that_is_not_published(): void
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra, ['status' => 'draft']);
        $user = $this->regularUser();
    ...
    public function test_user_can_initiate_and_webhook_completes_payment(): void
    {
        Http::fake([
            'api.xendit.co/v2/invoices' => Http::response([
                'id' => 'inv_glamping_test',
                'invoice_url' => 'https://checkout-staging.xendit.co/web/inv_glamping_test',
            ], 200),
        ]);
    ...
    public function test_user_cancelling_confirmed_glamping_booking_h2_or_more_gets_85_percent_refund(): void
    {
        Http::fake([
            'api.xendit.co/refunds' => Http::response(['id' => 'rfd_glamping_2', 'status' => 'SUCCEEDED'], 200),
        ]);
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra);
        $user = $this->regularUser();
        $booking = $this->paidBooking($user, $glamping, 'dikonfirmasi', now()->addDays(10));
    ...
    public function test_user_cancelling_confirmed_glamping_booking_within_h2_gets_zero_refund(): void
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra);
        $user = $this->regularUser();
        $booking = $this->paidBooking($user, $glamping, 'dikonfirmasi', now()->addDay());
    ...
    public function test_user_can_review_a_completed_glamping_booking(): void
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra);
        $user = $this->regularUser();
        $booking = $this->paidBooking($user, $glamping, 'selesai', now()->subDays(10));

        $response = $this->fromFrontend()->actingAs($user)
            ->postJson("/api/bookings/{$booking->id}/review", ['rating' => 5, 'comment' => 'Tendanya nyaman sekali']);

        $response->assertCreated();
        $this->assertDatabaseHas('reviews', [
            'booking_id' => $booking->id,
            'reviewable_type' => Glamping::class,
            'reviewable_id' => $glamping->id,
            'rating' => 5,
        ]);
    }

    public function test_public_can_list_reviews_for_a_published_glamping(): void
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra);
        $user = $this->regularUser();
        ...
        $response = $this->fromFrontend()->getJson("/api/glampings/{$glamping->slug}/reviews");

        $response->assertOk();
        $response->assertJsonFragment(['comment' => 'Oke banget']);
    }

    public function test_glamping_listing_shows_review_aggregates(): void
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra);
        $user = $this->regularUser();
        ...
        $response = $this->fromFrontend()->getJson("/api/glampings/{$glamping->slug}");

        $response->assertJsonPath('data.reviews_count', 1);
        $response->assertJsonPath('data.reviews_avg_rating', 4);
    }

    // --- Payout & mitra dashboard: cross-category (villa + glamping) correctness ---

    public function test_payout_batches_completed_bookings_across_villa_and_glamping(): void
    {
        Http::fake([
            'api.xendit.co/disbursements' => Http::response(['id' => 'disb_mixed_glamping_1', 'status' => 'COMPLETED'], 200),
        ]);
        $mitra = $this->approvedMitra();
        $villa = $mitra->mitraProfile->villas()->create([
            'name' => 'Villa Mixed Glamping', 'slug' => 'villa-mixed-glamping-'.uniqid(), 'city' => 'Bali',
    ...
    public function test_mitra_stats_include_glamping_bookings_alongside_villa(): void
    {
        $mitra = $this->approvedMitra();
        $glamping = $this->publishedGlamping($mitra);
        $this->paidBooking($this->regularUser(), $glamping, 'selesai', now()->subDays(20));

        $response = $this->fromFrontend()->actingAs($mitra)->getJson('/api/mitra/stats');

        $response->assertOk();
        $response->assertJsonPath('data.total_glampings', 1);
        $response->assertJsonPath('data.published_glampings', 1);
